...
def _eval_bounds(expr):
    ...
    # Strip balanced outer parens (repeatedly, in case of double-wrapping)
    while expr.startswith("(") and expr.endswith(")") and _outer_parens_balanced(expr):
        expr = expr[1:-1].strip()

    # Whole-expression "between" pattern — must run before AND-split shreds it
    ...
    # Top-level OR — drop missing-code branches; if multiple non-missing
    # branches remain, take the first (typical surveys put bounds in one
    # branch and missing codes in the others).
    ...
    # Top-level AND — intersect operand bounds
...
def _outer_parens_balanced(expr):
    """True if the leading `(` is matched by the trailing `)` (i.e. they
    form a single outer pair, not two unrelated groups)."""
...
def _split_top(expr, op):
    """Split `expr` on `op` ('and' or 'or') at parenthesis depth 0, with
    word-boundary handling so `and` inside `between N and M` (when the
    between is parenthesized) isn't seen at top level."""
    parts = []
    last = 0
    depth = 0
    op_len = len(op)
...
def _is_missing_code_clause(clause):
    """True if `clause` is a pure equality to a known missing-value sentinel."""
...
def _parse_atom(expr):
    """Parse a single comparison clause (a leaf of the boolean tree)."""
